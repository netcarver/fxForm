...
Built using some components and ideas from Will Bond's Flourish library and with a little
inspiration from ProcessWire by Ryan Cramer.
...
* Login to ProcessWire and go to Setup > Templates and hit the 'Add New Template' button. 
	Select contact.php and create a template called 'contact', add a title field (if not present) and save the template.
...
It should now be fairly trivial to add additional fields to your template for destination email address, SMTP server and password etc such that the
form's onSuccess method could use that information to send the contact details out for real.
